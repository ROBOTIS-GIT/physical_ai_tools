...
"""
CLI script to convert ROSbag recordings with MP4 videos to LeRobot v2.1 dataset format.

Usage:
    # Convert a single rosbag
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ROSbag recordings with MP4 videos to LeRobot v2.1 dataset format.",
...
Examples:
    ...
    # Log configuration
    logger.info("=" * 60)
    logger.info("ROSbag to LeRobot v2.1 Converter")
    logger.info("=" * 60)
    ...
    logger.info(f"Apply trim: {config.apply_trim}")
    logger.info(f"Apply exclude regions: {config.apply_exclude_regions}")
    logger.info("=" * 60)
