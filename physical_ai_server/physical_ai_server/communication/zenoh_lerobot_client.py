...
class LeRobotResponse:
    ...
    @classmethod
    def from_service_response(cls, response: Any, request_id: str = "") -> "LeRobotResponse":
        if response is None:
            return cls(
                success=False,
...
class ZenohLeRobotClient:
    ...
    def connect(self) -> bool:
        if ROS2ServiceClient is None:
            print("[ZenohLeRobotClient] zenoh_ros2_sdk not available")
            return False
            
        try:
            self._init_service_clients()
            self._connected = True
            print(f"[ZenohLeRobotClient] Connected to Zenoh router at {self.router_ip}:{self.router_port}")
            return True
    ...
    def _init_service_clients(self):
        ...
        for service_name, srv_type in service_configs:
            try:
                ...
                print(f"[ZenohLeRobotClient] Service client ready: {service_name}")
            except Exception as e:
                print(f"[ZenohLeRobotClient] Failed to create client for {service_name}: {e}")
    ...
    def _call_service(self, service_name: str, **kwargs) -> LeRobotResponse:
        ...
        try:
            print(f"[ZenohLeRobotClient] Calling {service_name} with {kwargs}")
            response = client.call(**kwargs)
            print(f"[ZenohLeRobotClient] Response from {service_name}: {response}")
            return LeRobotResponse.from_service_response(response)
        except Exception as e:
            print(f"[ZenohLeRobotClient] Service call failed: {e}")
            return LeRobotResponse(
                success=False,
    ...
    def subscribe_status(self, callback: Callable[[Dict], None]) -> bool:
        ...
        if ROS2Subscriber is None:
            print("[ZenohLeRobotClient] ROS2Subscriber not available")
            return False
        ...
        try:
            def on_progress(msg):
                ...
                print(f"[ZenohLeRobotClient] Progress received: step={data['step']}, loss={data['loss']:.4f}")
                callback(data)
            ...
            print(f"[ZenohLeRobotClient] Subscribed to progress topic: {self.TOPIC_PROGRESS}")
            return True
        except Exception as e:
            print(f"[ZenohLeRobotClient] Failed to subscribe to progress: {e}")
            return False
    ...
    def subscribe_actions(self, callback: Callable[[Dict], None]) -> bool:
        ...
        try:
            ...
            self._subscribers.append(sub)
            print(f"[ZenohLeRobotClient] Subscribed to action topic: {self.TOPIC_ACTION}")
            return True
        except Exception as e:
            print(f"[ZenohLeRobotClient] Failed to subscribe to actions: {e}")
            return False
    
    def subscribe_training_log(self, callback: Callable[[Dict], None]) -> bool:
        """Subscribe to training log - uses same progress topic."""
        # Training log data is included in progress updates
