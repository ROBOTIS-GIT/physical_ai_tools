...
# LeRobot-specific inference manager implementing InferenceBase interface
class LeRobotInference(InferenceBase):
    ...
    def load_policy(self) -> bool:
        # Load LeRobot policy from validated path
        try:
    ...
    def _preprocess(
        ...
        # Preprocess inputs for LeRobot policy
    ...
    def _convert_images2tensors(
        ...
        # Convert images to PyTorch tensors in LeRobot format
        processed_images = {}
    ...
    def _get_policy_class(self, name: str) -> PreTrainedPolicy:
        # Get LeRobot policy class by name
        if name == 'tdmpc':
    ...
    def get_available_policies(cls) -> List[str]:
        # Get list of available LeRobot policy types
        return [
            'tdmpc',
            'diffusion',
            'act',
