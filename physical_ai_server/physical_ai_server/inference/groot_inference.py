...
class GrootInference(InferenceBase):
    ...
    def get_saved_policies(cls) -> Tuple[List[str], List[str]]:
        # Get list of saved/cached GR00T policies
        # TODO: Implement logic to find GR00T models
        # They might be stored in NVIDIA Isaac specific directories
        
        saved_policy_path = []
        saved_policy_type = []
        
        # Example: Look in common NVIDIA Isaac/GR00T model directories
        possible_dirs = [
            os.path.expanduser('~/.isaac/models'),
            os.path.expanduser('~/.nvidia/groot_models'),
            '/opt/nvidia/isaac/models',
            '/workspace/isaac_models',  # Common in Docker containers
        ]
                        ...
                        for config_file in config_files:
                            ...
                            if os.path.exists(config_path):
                                try:
                                    with open(config_path, 'r') as f:
                                        config = json.load(f)
                                        policy_type = (config.get('model_type') or 
                                                     config.get('type') or 
                                                     config.get('groot_type'))
                                        if policy_type:
                                            saved_policy_path.append(item_path)
