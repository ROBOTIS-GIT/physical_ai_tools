...
class PhysicalAIServer(Node):
    ...
    def _init_core_components(self):
        ...
        self.file_browse_utils = FileBrowseUtils(
            max_workers=8, logger=self.get_logger()
        )
    ...
    def _init_ros_publisher(self):
        ...
        self.training_status_publisher = self.create_publisher(
            TrainingStatus, "/training/status", pub_qos_size
        )
    ...
    def _init_ros_service(self):
        ...
        service_definitions = [
            ...
            (
                "/get_saved_policies",
                GetSavedPolicyList,
                self.get_saved_policies_callback,
            ),
            (
                "/training/command",
                SendTrainingCommand,
                self.user_training_interaction_callback,
            ),
            (
                "/training/get_available_policy",
                GetPolicyList,
                self.get_available_list_callback,
            ),
            ("/training/get_user_list", GetUserList, self.get_user_list_callback),
            (
                "/training/get_dataset_list",
                GetDatasetList,
                self.get_dataset_list_callback,
            ),
            (
            ...
            (
                ...
                GetTrainingInfo,
                self.get_training_info_callback,
            ),
    ...
    def init_ros_params(self, robot_type):
        ...
        self.params = load_parameters(
            node=self, robot_type=robot_type, param_names=param_names
        )
        ...
        self.joint_order = load_parameters(
            node=self, robot_type=robot_type, param_names=self.joint_order_list
        )
    ...
    def init_robot_control_parameters_from_user_task(self, task_info):
        ...
        self.get_logger().info("Robot control parameters initialized successfully")
    ...
    def _handle_warmup_transition(self, previous_status: str):
        self.get_logger().info(
            f"Preparing rosbag recording, previous status: {previous_status}"
        )
    ...
    def _handle_run_transition(self, previous_status: str):
        self.get_logger().info(
            f"Starting rosbag recording, previous status: {previous_status}"
        )
    ...
    def _handle_save_transition(self, previous_status: str):
        self.get_logger().info(
            f"Stopping rosbag recording(save), previous status: {previous_status}"
        )
    ...
    def _handle_stop_transition(self, previous_status: str):
        self.get_logger().info(
            f"Stopping rosbag recording(stop), previous status: {previous_status}"
        )
    ...
    def _handle_finish_transition(self, previous_status: str):
        self.get_logger().info(
            f"Finishing rosbag recording, previous status: {previous_status}"
        )
    ...
    def _data_collection_timer_callback(self):
        ...
        if camera_msgs is None:
            if (
                time.perf_counter() - self.start_recording_time
    ...
    def _inference_timer_callback(self):
        ...
        if camera_msgs is None or len(camera_msgs) != len(
            self.params["camera_topic_list"]
        ):
            self.get_logger().info("Waiting for camera data...")
            return
        elif follower_msgs is None:
        ...
        try:
            ...
            action_pub_msgs = self.data_manager.data_converter.tensor_array2joint_msgs(
                action, self.joint_topic_types, self.joint_order
            )
    ...
    def _validate_training_request(
        self, resume, resume_model_path, output_folder_name, weight_save_root_path
    ):
        """
    ...
    def get_user_list_callback(self, request, response):
        try:
            ...
            folder_names = [
                name
                for name in os.listdir(self.DEFAULT_SAVE_ROOT_PATH)
    ...
    def get_dataset_list_callback(self, request, response):
        ...
        try:
            ...
            dataset_names = [
                name for name in os.listdir(user_path) if (user_path / name).is_dir()
            ]
    ...
    def get_model_weight_list_callback(self, request, response):
        ...
        try:
            ...
            model_folders = [f.name for f in save_root_path.iterdir() if f.is_dir()]
    ...
    def get_training_info_callback(self, request, response):
        ...
        try:
            ...
            try:
                ...
                response.success = True
                response.message = f"Training configuration loaded successfully from {train_config_path}"
        ...
        try:
            ...
            if self.hf_api_worker.start():
                ...
                self.hf_status_publisher = self.create_publisher(
                    HFOperationStatus, "/huggingface/status", self.PUB_QOS_SIZE
                )
            else:
    ...
    def _hf_status_timer_callback(self):
        ...
        try:
            ...
            last_status = (
                self._last_hf_status.get("status", "Unknown")
                if hasattr(self, "_last_hf_status")
                else "Unknown"
            )
            ...
            if request.action == "get_path":
                ...
                if target_files or target_folders:
                    result = self.file_browse_utils.handle_go_parent_with_target_check(
                        request.current_path, target_files, target_folders
                    )
                else:
    ...
    def handle_joystick_trigger(self, joystick_mode: str):
        self.get_logger().info(f"Joystick mode updated: {joystick_mode}")
        ...
        if joystick_mode == "right":
            self.get_logger().info("Right tact triggered - Moving to next episode")
            if len(self.data_manager.get_task_info().task_instruction) > 1:
                self.data_manager.record_next_episode()
            else:
                self.data_manager.record_early_save()
        elif joystick_mode == "left":
            self.get_logger().info("Left tact triggered - Re-record current episode")
            self.data_manager.re_record()
        elif joystick_mode == "right_long_time":
            self.get_logger().info("Right long tact triggered - Custom")
            # If you want, you can add custom functionality.
        elif joystick_mode == "left_long_time":
            self.get_logger().info("Left long tact triggered - Custom")
            # If you want, you can add custom functionality.
        else:
            self.get_logger().info(f"Received joystick trigger: {joystick_mode}")
