...
##############################1063 change########
        # Subscriber for live task instruction updates during inference
        self.update_instruction_sub = self.create_subscription(
            String,
            '/update_task_instruction',
            self._on_update_task_instruction,
            10,
        )

    def _on_update_task_instruction(self, msg: String):
        """Update the running inference manager's task instruction."""
        if self.inference_manager is None:
            self.get_logger().warn(
                'Received /update_task_instruction but no inference is running'
            )
            return
