...
class PhysicalAIServer(Node):
    ...
    def __init__(self):
        ...
        self.service_manager_factory = ServiceManagerFactory(node=self, main_server=self)
        self.service_managers = self.service_manager_factory.create_all_managers()
...
def main(args=None):
    ...
    try:
        ...
        # Cleanup service managers and HF API Worker before destroying node
        if hasattr(node, 'service_manager_factory'):
            node.service_manager_factory.cleanup_all_managers()
