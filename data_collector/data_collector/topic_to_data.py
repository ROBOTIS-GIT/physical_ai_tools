...
class RobotDataSubscriber(Node):
    def __init__(self):
        ...
        self.follower_arm_right = Subscriber(self, JointState, '/joint_trajectory_right/joint_trajectory')
        self.follower_arm_left = Subscriber(self, JointState, '/joint_trajectory_left/joint_trajectory')
        ...
        self.leader_arm_right = Subscriber(self, JointState, '/leader/joint_trajectory_right/joint_trajectory')
        self.leader_arm_left = Subscriber(self, JointState, '/leader/joint_trajectory_left/joint_trajectory')
        self.leader_neck = Subscriber(self, JointState, '/leader/neck_controller/joint_trajectory')
        self.leader_linear = Subscriber(self, JointState, '/leader/body_controller/joint_trajectory')
    ...
    def synced_callback(
        ...
        try:
            r_hand_pos_map = dict(zip(follower_hand_right_msg.name, follower_hand_right_msg.position))
            ordered_follower_r_hand = [r_hand_pos_map[name] for name in self.joint_order_follower_r_hand]
            l_hand_pos_map = dict(zip(follower_hand_left_msg.name, follower_hand_left_msg.position))
            ordered_follower_l_hand = [l_hand_pos_map[name] for name in self.joint_order_follower_l_hand]
            r_arm_pos_map = dict(zip(follower_arm_right_msg.name, follower_arm_right_msg.position))
            ordered_follower_r_arm = [r_arm_pos_map[name] for name in self.joint_order_follower_r_arm]
            l_arm_pos_map = dict(zip(follower_arm_left_msg.name, follower_arm_left_msg.position))
            ordered_follower_l_arm = [l_arm_pos_map[name] for name in self.joint_order_follower_l_arm]
            obs_pos = (
